...
static Ref_t createDetector(Detector& lccdd, xml_h e, SensitiveDetector /*sens*/) {
  ...
  Info("BackwardsVacBox", "win_xmin: %f", win_xmin);
  ...
  //Info("BackwardsVacBox", "zQT: %f", zQT);
  //Info("BackwardsVacBox", "xQT: %f", xQT);
  //Info("BackwardsVacBox", "zQB: %f", zQB);
  //Info("BackwardsVacBox", "xQB: %f", xQB);

  //double zB = points.attr<double>( _Unicode(zB) );
  //double xB = points.attr<double>( _Unicode(xB) );

  //Info("BackwardsVacBox", "zB: %f", zB);
  //Info("BackwardsVacBox", "xB: %f", xB);

  //auxiliary point from direct calculation
  //double xA = xQB + (zB-win_z)*(xB-xQB)/(zB-zQB);
  //double xA = xQB + (win_z-zQB)*(xB-xQB)/(zB-zQB);
  //win_xmin = xA;
  //Info("BackwardsVacBox", "xA: %f", xA);
  ...
  //end of B2BeR magnet
  double b2b_end_xy = 200*mm; // full size in x and y at the end of B2eR, equal to inner diameter
